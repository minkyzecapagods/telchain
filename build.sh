...
jar cfm telchain-server.jar /tmp/server-manifest.mf -C "$OUT_DIR" .
echo "[BUILD] ✓ telchain-server.jar"

# Sensor JAR
echo "Main-Class: $MAIN_SENSOR" > /tmp/sensor-manifest.mf
jar cfm telchain-sensor.jar /tmp/sensor-manifest.mf -C "$OUT_DIR" .
echo "[BUILD] ✓ telchain-sensor.jar"

# Dashboard JAR
echo "Main-Class: $MAIN_DASH" > /tmp/dash-manifest.mf
jar cfm telchain-dashboard.jar /tmp/dash-manifest.mf -C "$OUT_DIR" .
...
echo "  java -jar telchain-server.jar"
echo ""
echo "  # Terminal 2 - Start sensor 1:"
echo "  java -jar telchain-sensor.jar localhost 7070 sensor-01 s3cr3t 3000"
echo ""
echo "  # Terminal 3 - Start sensor 2:"
echo "  java -jar telchain-sensor.jar localhost 7070 sensor-02 s3cr3t2 4000"
echo ""
echo "  # Terminal 4 - Start dashboard:"
echo "  java -jar telchain-dashboard.jar localhost 7070 dash-01 d4shp4ss"
echo "========================================"